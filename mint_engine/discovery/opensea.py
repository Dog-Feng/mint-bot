...
async def resolve_opensea(url_or_slug: str) -> OpenSeaPreview:
    ...
    async with httpx.AsyncClient(timeout=20.0, headers=headers, follow_redirects=True) as client:
        ...
        slug = parsed["slug"]
        collection = await _get(client, f"https://api.opensea.io/api/v2/collections/{slug}")
        drop = await _get_optional(client, f"https://api.opensea.io/api/v2/drops/{slug}")
...
async def _get(client: httpx.AsyncClient, url: str) -> dict:
    ...
    if response.status_code >= 400:
        raise ConfigError(f"OpenSea API {response.status_code}: {response.text[:200]}")
    data = response.json()
