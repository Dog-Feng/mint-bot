...
from mint_engine.chase import (
    ChaseContext,
    ChaseWallet,
    init_chase_wallets,
    is_terminal_chase_stage,
)
from mint_engine.wallet_mint_count import read_wallet_mint_count
...
from mint_engine.discovery.opensea_stages import (
    ...
    stage_bounds,
    stage_group_key,
    stage_max_per_wallet,
    stage_window_open,
    strictest_max_per_wallet,
...
PUBLIC_BLIND_RECEIPT_POLL_INTERVAL_SEC = 0.15
PUBLIC_BLIND_OBSERVE_MAX_SEC = 45.0
...
class MintController:
    ...
    async def mint(self, cancel: asyncio.Event | None = None) -> dict[str, Any]:
        ...
        if multi_stage:
            if not self.opensea.stages:
                raise ConfigError("refusing to mint: OpenSea drop has no stages")
            if not build_stage_sequence(self.opensea.stages):
                raise ConfigError("refusing to mint: no eligible drop stages")
            if not drop_has_future_mint_window(self.opensea.stages, now):
                raise ConfigError("refusing to mint: drop has no future mint window")
        ...
        if self.config.safety.dry_run_required and not dry["simulation"]["ok"]:
            if not dry["simulation"].get("expected"):
                raise ConfigError(f"dry run failed: {dry['simulation']['message']}")
        self._assert_quantity_for_sale(self._run_max_per_wallet(report, multi_stage))

        ready = [w for w in self.wallets.signers()]
        if not ready:
            raise ConfigError("no wallet with a private key")
        ready, skipped_minted = await self._partition_already_minted(report, ready)
        for row in skipped_minted:
            self.log(f"[SKIP] {row['wallet']} already minted: {row.get('error')}")
        if not ready:
            raise ConfigError("no wallet eligible to mint (all skipped or already minted)")
        ...
        try:
            if multi_stage:
                payload = await self._run_stage_chase(report, ready, pre_skipped=skipped_minted)
            else:
                payload = await self._run_chain_chase(report, ready, pre_skipped=skipped_minted)
        except RunCancelled:
    ...
    async def _run_chain_chase(
        self,
        report: InspectReport,
        ready,
        *,
        pre_skipped: list[dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
        while True:
            self._check_cancel()
            ...
            break
        self._check_cancel()
        results = list(pre_skipped or []) + await self._blast_all_wallets(report, ready)
        return {"report": report.model_dump(), "results": results, "chase": None}

    async def _run_stage_chase(
        self,
        report: InspectReport,
        ready,
        *,
        ...
        async def on_public_blind(stage_start: int, launch_report: InspectReport) -> None:
            nonlocal analysis, cached_quote
            prearm_lead = float(schedule.public_blind_prearm_lead_sec)
            blast_lead = float(schedule.public_blind_blast_lead_sec)
            k_nonces = int(schedule.public_blind_nonce_count or 0)
            send_gap = max(0.0, float(schedule.public_blind_send_gap_sec))
            if prearm_lead <= 0 or blast_lead <= 0 or k_nonces < 1:
                self.log("[BLIND] disabled: prearm/blast lead must be > 0 and nonce_count >= 1")
                return
            if blast_lead >= prearm_lead:
                self.log(
                    "[BLIND] warning: blast_lead should be < prearm_lead "
                    f"(blast={blast_lead:g} prearm={prearm_lead:g})"
                )
            ...
            prearm_at = stage_start - prearm_lead
            if time.time() < prearm_at:
                await self._wait_until(prearm_at, "BLIND PRE-ARM")
            if analysis is None:
                analysis = await self._analyze()
            chain_sale = analysis.get("sale")
            meta = (analysis or {}).get("meta") or {}
            ...
            self.log(
                f"[BLIND] PRE-ARM gas_limit={gas_limit} wallets={len(blind_batch)} "
                f"nonces={k_nonces} T-{prearm_lead:g}s"
            )

            async def prearm_one(cw: ChaseWallet) -> None:
                if cw.done:
                    return
                ...
                rpc_url = url_map.get(cw.address.lower())
                try:
                    base_nonce = await self.pool.get_nonce(
                        cw.wallet.address, "pending", prefer=rpc_url
                    )
                except Exception as exc:
                    self.log(f"[BLIND] {cw.label} nonce read failed: {exc}")
                    cw.finish(
                        {
                            "wallet": cw.label,
                            "address": cw.address,
                ...
                try:
                    ...
                    self.log(f"[BLIND] {cw.label} PRE-ARM sign failed: {exc}")
                    cw.finish(
                        {
                            "wallet": cw.label,
                            "address": cw.address,
                    ...
                    return
                cw.blind_prepared = ladder
                self.log(
                    f"[BLIND] {cw.label} PRE-ARM signed {len(ladder)} tx "
                    f"nonce={base_nonce}..{base_nonce + len(ladder) - 1}"
                )

            await asyncio.gather(*(prearm_one(cw) for cw in blind_batch))

            blast_at = stage_start - blast_lead
            if time.time() < blast_at:
                await self._wait_until(blast_at, "BLIND BLAST")
            self.log(
                f"[BLIND] blast send x{k_nonces} per wallet T-{blast_lead:g}s (no resign/retry)"
            )

            async def blast_send_one(cw: ChaseWallet) -> None:
                if cw.done or not cw.blind_prepared:
                    return
                for idx, prepared in enumerate(cw.blind_prepared):
                    if self._user_cancelled():
                        return
                    last = await self._send_prepared(
                        ...
                        prepared,
                        stop=stop,
                        receipt_timeout_sec=0.01,
                    )
                    status = str(last.get("status") or "")
                    if status == "SEND_FAILED":
                        self.log(
                            f"[BLIND] {cw.label} send {idx + 1}/{k_nonces} SEND_FAILED "
                            f"{last.get('error') or ''}"
                        )
                    else:
                        self.log(
                            f"[BLIND] {cw.label} send {idx + 1}/{k_nonces} "
                            f"tx={prepared.get('tx_hash')}"
                        )
                    if send_gap > 0 and idx + 1 < len(cw.blind_prepared):
                        await asyncio.sleep(send_gap)

            await asyncio.gather(*(blast_send_one(cw) for cw in blind_batch))

            observe_deadline = min(
                time.time() + PUBLIC_BLIND_OBSERVE_MAX_SEC,
                stage_start + PUBLIC_BLIND_OBSERVE_MAX_SEC,
            )
            self.log("[BLIND] observe receipts (short poll, no resend)")

            async def observe_one(cw: ChaseWallet) -> None:
                if cw.done or not cw.blind_prepared:
                    return
                receipts: list[dict[str, Any] | None] = [None] * len(cw.blind_prepared)
                while time.time() < observe_deadline:
                    if self._user_cancelled():
                        cw.finish(self._skip_cancelled(stage_report, cw.wallet), "CANCELLED")
                        return
                    pending = False
                    for i, prepared in enumerate(cw.blind_prepared):
                        if receipts[i] is not None:
                            continue
                        tx_hash = prepared.get("tx_hash")
                        if not tx_hash:
                            continue
                        receipt = await self.pool.wait_receipt(
                            tx_hash,
                            timeout=PUBLIC_BLIND_RECEIPT_POLL_INTERVAL_SEC,
                    ...
                    if not pending:
                        break
                    if any(
                        receipts[i] is not None and _hex_int(receipts[i].get("status")) == 1
                        for i in range(len(receipts))
                    ):
                        break
                    await asyncio.sleep(PUBLIC_BLIND_RECEIPT_POLL_INTERVAL_SEC)
                ...
                for i, prepared in enumerate(cw.blind_prepared):
                    ...
                    if receipt is None:
                        continue
                    item = await self._result_from_receipt(
                        stage_report,
                        prepared,
                        receipt,
                        prepared.get("broadcasts") or [],
                    )
                    if item.get("sold_out"):
                        note_sold_out(cw.address, str(item.get("error") or "sold out"))
                    if item.get("status") == "SUCCESS":
                        best = item
                        break
                    best = item
                if best is None:
                    last_prepared = cw.blind_prepared[-1]
                    cw.finish(
                        {
                            "wallet": cw.label,
                            "address": cw.address,
                            "method": self._result_method(stage_report),
                            "quantity": self.config.mint.quantity,
                            "tx_hash": last_prepared.get("tx_hash"),
                            "status": "TIMEOUT",
                            "error": "blind observe: no receipt",
                            "chase_status": "BLIND_OBSERVE",
                        },
                        "BLIND_OBSERVE",
                    )
                    return
                if best.get("sold_out"):
                    cw.finish(best, str(best.get("status") or "SOLD_OUT"))
                    return
                cw.finish(best, str(best.get("status") or "ERROR"))
                if best.get("status") == "SUCCESS":
                    self.log(f"[BLIND] {cw.label} SUCCESS tx={best.get('tx_hash')}")

            await asyncio.gather(*(observe_one(cw) for cw in blind_batch))
            for cw in blind_batch:
                cw.blind_prepared = None
        ...
        while ctx.active_wallets:
            ...
            stage_groups = self._probe_batch_stage_groups(ctx, probe_batch)
            any_hot = False
            tick_stop = False
            ...
            for probe_stage, group_batch in stage_groups:
                wall = int(time.time())
                hot = bool(
                    probe_stage
                    and hot_path_active(
                ...
                public_only = all(
                    use_chain_public_mint(ctx.current_stage(cw))
                    for cw in group_batch
                    if ctx.current_stage(cw) is not None
                )
                if hot:
                        ...
                        if use_chain_public_mint(stage):
                            ...
                            if prepared is not None:
                                ...
                                if probe is False:
                                    ...
                                    if is_terminal_chase_stage(ctx, chase_wallet, stage):
                                        chase_wallet.finish(
                                            {
                                                "wallet": wallet.label,
                                                "address": wallet.address,
                                                "status": "SKIP",
                                                "error": f"not eligible for final stage [{label}]",
                                ...
                                if item.get("sold_out"):
                                    note_sold_out(
                                        wallet.address, str(item.get("error") or "sold out")
                                    )
                            ...
                            mint_probe = await self._probe_opensea_mint_cached(
                                chase_wallet, wallet.address
                            )
                ...
                for item in await asyncio.gather(
                    *(handle(cw) for cw in group_batch),
            ...
            if not any_hot:
                hot_announced = False
            await maybe_sync_stages(hot=any_hot)
            await asyncio.sleep(0.05)

        results = list(pre_skipped or []) + [w.result for w in ctx.wallets if w.result]
        chase_summary = [
            {
                "wallet": w.label,
                "address": w.address,
    ...
    async def _on_opensea_not_eligible(
        ...
        *,
        ...
        if probe.sold_out:
            ...
            if probe.drop_fully_sold_out or is_terminal_chase_stage(ctx, chase_wallet, stage):
    ...
    async def _probe_opensea_mint_cached(
        self, chase_wallet: ChaseWallet, address: str
    ) -> OpenSeaMintProbe:
        ttl = float(self.config.schedule.opensea_plan_ttl_sec)
        now = time.time()
        cached = chase_wallet.opensea_probe_cached
        if (
            cached is not None
            and ttl > 0
            and cached.ok is True
            and cached.plan is not None
            and now - chase_wallet.opensea_probe_cached_at < ttl
        ):
            return cached
        probe = await self._probe_opensea_mint(address)
        if probe.ok is True and probe.plan is not None and ttl > 0:
            chase_wallet.opensea_probe_cached = probe
            chase_wallet.opensea_probe_cached_at = now
        else:
            chase_wallet.opensea_probe_cached = None
            chase_wallet.opensea_probe_cached_at = 0.0
        return probe
    ...
    async def _wallet_states(self, sale, value: int, gas_reserve: int) -> list[WalletReady]:
        ...
        async def one(wallet, rpc_url: str | None) -> WalletReady:
            ...
            max_cap = sale.max_per_wallet if sale else None
            if self.opensea and self.opensea.stages:
                stage_cap = strictest_max_per_wallet(self.opensea.stages)
                if stage_cap is not None:
                    max_cap = stage_cap if max_cap is None else min(max_cap, stage_cap)
            if max_cap is not None and max_cap < self.config.mint.quantity:
                status = WalletStatus.WALLET_LIMIT
                note = f"quantity {self.config.mint.quantity} > maxPerWallet {max_cap}"
            return WalletReady(
                label=wallet.label,
                address=wallet.address,
                status=status,
    ...
    def _probe_batch_stage_groups(
        self,
        ctx: ChaseContext,
        probe_batch: list[ChaseWallet],
    ) -> list[tuple[dict[str, Any], list[ChaseWallet]]]:
        order: list[str] = []
        buckets: dict[str, list[ChaseWallet]] = {}
        for chase_wallet in probe_batch:
            stage = ctx.current_stage(chase_wallet)
            key = stage_group_key(stage)
            if key not in buckets:
                buckets[key] = []
                order.append(key)
            buckets[key].append(chase_wallet)
        groups: list[tuple[dict[str, Any], list[ChaseWallet]]] = []
        for key in order:
            batch = buckets[key]
            stage = ctx.current_stage(batch[0])
            if stage is not None:
                groups.append((stage, batch))
        return groups

    def _run_max_per_wallet(self, report: InspectReport, multi_stage: bool) -> int | None:
        if multi_stage and self.opensea and self.opensea.stages:
            return strictest_max_per_wallet(self.opensea.stages)
        return report.sale.max_per_wallet

    def _assert_quantity_for_sale(self, max_per_wallet: int | None) -> None:
        qty = self.config.mint.quantity
        if max_per_wallet is not None and qty > max_per_wallet:
            raise ConfigError(
                f"quantity={qty} > maxPerWallet={max_per_wallet} (strictest limit for this run)"
            )

    def _quantity_error_for_stage(self, stage: dict[str, Any], quantity: int) -> str | None:
        cap = stage_max_per_wallet(stage)
        if cap is not None and quantity > cap:
            label = (stage.get("label") or stage.get("stage_type") or "stage").strip()
            return f"quantity={quantity} > maxPerWallet={cap} for stage [{label}]"
        return None

    def _mint_count_target(self, report: InspectReport) -> str:
        impl = report.implementation
        if impl:
            return checksum(impl)
        return checksum(report.contract)

    async def _partition_already_minted(
        self,
        report: InspectReport,
        ready: list,
    ) -> tuple[list, list[dict[str, Any]]]:
        if not self.config.wallets.skip_if_already_minted:
            return ready, []
        max_cap = self._run_max_per_wallet(
            report, bool(self.opensea and self.opensea.stages)
        )
        target = self._mint_count_target(report)
        kept = []
        skipped: list[dict[str, Any]] = []
        qty = self.config.mint.quantity
        for wallet in ready:
            rpc_url = self._shard_url_for(wallet.address)
            count = await read_wallet_mint_count(
                self.pool, target, wallet.address, prefer=rpc_url
            )
            if count is None:
                kept.append(wallet)
                continue
            ...
            if max_cap is not None:
                if count >= max_cap:
                    reason = f"on-chain mint count {count} >= maxPerWallet {max_cap}"
                elif count + qty > max_cap:
                    reason = (
                        f"on-chain mint count {count} + quantity {qty} "
                        f"> maxPerWallet {max_cap}"
                    )
            elif count > 0:
                reason = f"on-chain mint count {count} > 0 (skip_if_already_minted)"
            if reason:
                skipped.append(
                    {
                        "wallet": wallet.label,
