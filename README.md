...
Dry Run（eth_call，不广播）
        ↓
启动（阶段追逐）：按 OpenSea 阶段序等待各阶段开始（PREPARE_LEAD / T−20 预分析+Gas / T−5 FINAL 刷新 / ARMED）。开点后 **热路径**（默认开，未暴露页面）：少做 full refresh，预售先 `POST /drops/.../mint`（日志 `[OPENSEA MINT] HTTP …`），200 复用 TxPlan 不再二次 /mint；422 在阶段窗内约 1s 重试。公开轮 T−1 预签 estimateGas+sign，到点 eth_call 通过后立刻广播预签 tx。每钱包用 mint API 或链上 simulate 探测资格 → 无资格等下一阶段；公开轮为终局。回执回来再发该节点下一钱包
        ↓
